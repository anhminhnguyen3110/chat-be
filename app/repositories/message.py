...
class MessageRepository(BaseRepository[Message]):
    ...
    async def get_by_session_id(self, session_id: int, skip: int = 0, limit: int = 100) -> List[Message]:
        """Get messages for a session, ordered by created_at ascending (oldest first)."""
        result = await self.session.execute(
            select(Message)
    ...
    async def get_by_session_id_paginated(
        self, 
        session_id: int, 
        skip: int = 0,
        limit: int = 20
        ...
        """
        Get messages for a session with pagination (newest first).
        ...
        Returns:
            List of messages ordered by created_at DESC (newest first)
        """
        query = (
            select(Message)
            .where(Message.session_id == session_id)
